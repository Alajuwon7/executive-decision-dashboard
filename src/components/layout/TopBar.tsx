...
import { Download, Plus, LogOut, BrainCircuit, RotateCcw } from "lucide-react";
...
export function TopBar({ onAddEntry }: TopBarProps) {
  ...
  const handleResetLayout = () => {
    if (confirm("Reset widget layout to defaults? Your current arrangement will be lost.")) {
      resetLayout();
    }
  };
  ...
  return (
    <header className="flex items-center justify-between px-6 py-4 border-b border-border bg-bg/80 backdrop-blur-sm sticky top-0 z-40">
      ...
      <div className="flex items-center gap-3">
        <Button variant="secondary" size="sm" onClick={handleResetLayout} title="Reset widget layout">
